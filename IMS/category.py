...
class categoryClass:
    def __init__(self,root):
        ...
        title=Label(self.root,text="Manage Product Category",font=("goudy old style",30,"bold"),bg="#184a45",fg="white",bd=3,relief=RIDGE).pack(side=TOP,fill=X,padx=10,pady=20)
        ...
        self.im2=Image.open("images/pd.jpg")
    ...
    def add(self):
        ...
        try:
            if self.var_name.get()=="":
                ...
                row=cur.fetchone()
                if row!=None:
                    messagebox.showerror("Error","Category already presnt, try different",parent=self.root)
                else:
